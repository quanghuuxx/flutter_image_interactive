...
String potrace(
  img.Image src, {
  Turnpolicy turnpolicy = Turnpolicy.minority,
  int turdsize = 2,
  bool optcurve = true,
  int alphamax = 1,
  ...
  return _GetSVG(bm, "curve", pathlist).run();
}
...
class _Bitmap {
  ...
  _Bitmap.fromImg(img.Image image)
    ...
    final hHaft = h / 2, wHaft = w / 2;
    for (var y = 0, yr = h - 1; y <= hHaft; y++, yr--) {
      for (var x = 0, xr = w - 1; x <= wHaft; x++, xr--) {
        _checkPixel(image.getPixel(x, y));
        _checkPixel(image.getPixel(xr, y));
        _checkPixel(image.getPixel(x, yr));
        _checkPixel(image.getPixel(xr, yr));
      }
    }
  }

  void _checkPixel(img.Pixel pixel) {
    num color = 0.2126 * pixel.r + 0.7153 * pixel.g + 0.0721 * pixel.b;
    final index = pixel.y * w + pixel.x;
    data[index] = (color < 128 ? 1 : 0);
  }
  ...
  _Bitmap copy() {
    return _Bitmap(w, h)..data = Uint8List.fromList(data);
  }
}
...
class _Quad {
  List<double> data = List.filled(9, 0);
  double at(int x, int y) => data[x * 3 + y];
}

class _Opti {
  double pen = 0, t = 0, s = 0, alpha = 0;
...
class _Sum {
  ...
  @override
  String toString() {
    return '_Sum(x:$x y:$y xy:$xy x2:$x2 y2:$y2)';
  }
}
...
class _BitmapToPathlist {
  ...
  _Point? findNext(_Point point) {
    int i = (bm1.w * point.y + point.x).toInt();
  ...
  _Path findPath(_Point point) {
    _Path path = _Path();
    int x = point.x.toInt(), y = point.y.toInt(), dirx = 0, diry = 1, tmp;
    ...
    while (true) {
      ...
      if (r && !l) {
        if (turnpolicy == Turnpolicy.right ||
            (turnpolicy == Turnpolicy.black && path.sign == turnpolicy.sign) ||
            (turnpolicy == Turnpolicy.white && path.sign == turnpolicy.sign) ||
            (turnpolicy == Turnpolicy.majority && majority(x, y)) ||
            (turnpolicy == Turnpolicy.minority && !majority(x, y))) {
          tmp = dirx;
          dirx = -diry;
          diry = tmp;
        } else {
  ...
  List<_Path> run() {
    ...
    _Point? currentPoint;
    while ((currentPoint = findNext(currentPoint ??= _Point())) != null) {
...
class _ProcessPath {
  ...
  static int xprod(_Point p1, _Point p2) {
    return (p1.x * p2.y - p1.y * p2.x).toInt();
  }
  ...
  static double quadform(_Quad Q, _DPoint w) {
    List<double> v = <double>[w.x, w.y, 1];
    double sum = 0.0;
  ...
  static _DPoint interval(double lambda, _DPoint a, _DPoint b) {
  ...
  static double ddenom(_DPoint p0, _DPoint p2) {
    _Point r = dorthInfty(p0, p2);
    return r.y * (p2.x - p0.x) - r.x * (p2.y - p0.y);
  }

  static double dpara(_DPoint p0, _DPoint p1, _DPoint p2) {
    double x1 = p1.x - p0.x;
    double y1 = p1.y - p0.y;
    double x2 = p2.x - p0.x;
    double y2 = p2.y - p0.y;
  ...
  static double cprod(_DPoint p0, _DPoint p1, _DPoint p2, _DPoint p3) {
    double x1 = p1.x - p0.x;
    double y1 = p1.y - p0.y;
    double x2 = p3.x - p2.x;
    double y2 = p3.y - p2.y;
  ...
  static double iprod(_DPoint p0, _DPoint p1, _DPoint p2) {
    double x1 = p1.x - p0.x;
    double y1 = p1.y - p0.y;
    double x2 = p2.x - p0.x;
    double y2 = p2.y - p0.y;
  ...
  static double iprod1(_DPoint p0, _DPoint p1, _DPoint p2, _DPoint p3) {
    double x1 = p1.x - p0.x;
    double y1 = p1.y - p0.y;
    double x2 = p3.x - p2.x;
    double y2 = p3.y - p2.y;
  ...
  static double ddist(_DPoint p, _DPoint q) {
    return math.sqrt((p.x - q.x) * (p.x - q.x) + (p.y - q.y) * (p.y - q.y));
  }

  static _DPoint bezier(double t, _DPoint p0, _DPoint p1, _DPoint p2, _DPoint p3) {
    final s = 1 - t;
    return _DPoint(
  ...
  static double tangent(_DPoint p0, _DPoint p1, _DPoint p2, _DPoint p3, _DPoint q0, _DPoint q1) {
    final A = cprod(p0, p1, q0, q1);
    final B = cprod(p1, p2, q0, q1);
    final C = cprod(p2, p3, q0, q1);
  ...
  static void calcLon(_Path path) {
    ...
    List<int> pivk = List.filled(n, 0), nc = List.filled(n, 0), ct = List.filled(4, 0);
  ...
  static double penalty3(_Path path, int i, int j) {
    int n = path.len;
    List<_Point> pt = path.pt;
    List<_Sum> sums = path.sums;
    int x, y, xy, x2, y2, k, ex, ey;
    double a, b, c, s, px, py;
    bool r = false;
  ...
  static void bestPolygon(_Path path) {
    ...
    List<double> pen = List.filled(n + 1, 0);
    List<int> prev = List.filled(n + 1, 0),
        clip0 = List.filled(n, 0),
        clip1 = List.filled(n + 1, 0),
        // (quanghuuxx): phải khởi tạo cho seg0 như này thì khi chạy release mới hoạt động đúng ? thử điều tra xem tại sao
        seg0 = List.generate(n + 1, (_) => _),
  ...
  static void adjustVertices(_Path path) {
    int n = path.len, m = path.m.toInt(), x0 = path.x0.toInt(), y0 = path.y0.toInt();
    List<int> po = path.po;
    List<_Point> pt = path.pt;
    List<_DPoint> ctr = List.filled(m, _DPoint()), dir = List.filled(m, _DPoint());
  ...
  void optiCurve(_Path path) {
    ...
    List<int> pt = List.filled(m + 1, 0), len = List.filled(m + 1, 0), convc = List.filled(m, 0);
    List<double> pen = List.filled(m + 1, 0), areac = List.filled(m + 1, 0);
    ...
    om = len[m];
    ocurve = _Curve(om);
    List<double> s = List.filled(om, 0), t = List.filled(om, 0);
...
class _GetSVG {
  ...
  String optType;
  List<_Path> pathlist;
  _GetSVG(this.bm, this.optType, this.pathlist);

  String bezier(_Curve curve, int i) {
    return 'C ${(curve.c[i * 3 + 0].x).toStringAsFixed(3)} ${(curve.c[i * 3 + 0].y).toStringAsFixed(3)},${(curve.c[i * 3 + 1].x).toStringAsFixed(3)} ${(curve.c[i * 3 + 1].y).toStringAsFixed(3)},${(curve.c[i * 3 + 2].x).toStringAsFixed(3)} ${(curve.c[i * 3 + 2].y).toStringAsFixed(3)} ';
  }

  String segment(_Curve curve, int i) {
    return 'L ${(curve.c[i * 3 + 1].x).toStringAsFixed(3)} ${(curve.c[i * 3 + 1].y).toStringAsFixed(3)} ${(curve.c[i * 3 + 2].x).toStringAsFixed(3)} ${(curve.c[i * 3 + 2].y).toStringAsFixed(3)} ';
  }

  String path(_Curve curve) {
    final n = curve.n;
    String p =
        'M${(curve.c[(n - 1) * 3 + 2].x).toStringAsFixed(3)} ${(curve.c[(n - 1) * 3 + 2].y).toStringAsFixed(3)} ';
